...
public class MapboxGPSControllerMobile : MonoBehaviour
{
    ...
    public float maxAllowedAccuracy = 30f; // in meters
...
        mapCenter = new Vector2d(40.7128, -74.0060); // NYC default
...
        if (!Input.location.isEnabledByUser)
        ...
        Input.location.Start(1f, 1f); // 1m accuracy, 1m distance update
...
    void UpdateGPS()
    {
        ...
        // double distance = Vector2d.Distance(latestRaw, gpsLatLon) * 111000.0; // meters
        double distance = Conversions.LatLonToMeters(
    gpsLatLon.x, gpsLatLon.y,
    latestRaw.x, latestRaw.y
);




        // Update even for tiny movements
        // if (distance > 0.05f || horizontalAccuracy < Input.location.lastData.horizontalAccuracy)
        if (distance < 2.0f) return; // ignore GPS noise

        gpsLatLon = latestRaw;

        // Smooth player movement
        float smoothFactor = Mathf.Clamp01(gpsSmoothSpeed * Time.deltaTime * 2f); // faster smoothing for mobile
